...
def test_source_project_and_installed_distribution_versions_match() -> None:
    project = _project()
    expected = project["version"]
    assert growthevo.__version__ == expected
    assert importlib.metadata.version("growthevo-harness") == expected
...
def test_locked_ope_cli_reports_version_without_loading_benchmark_args(
    ...
    assert capsys.readouterr().out == f"growthevo-locked-ope {growthevo.__version__}\n"
...
def test_locked_targeting_cli_reports_version_without_loading_benchmark_args(
    ...
    assert capsys.readouterr().out == (
        f"growthevo-locked-targeting {growthevo.__version__}\n"
    )
